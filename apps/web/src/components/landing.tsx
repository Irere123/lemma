  ...
  return (
    <header className='mb-6'>
      <div className='flex flex-col justify-center gap-4'>
        <img src={avatarSrc} alt={alt} className='h-12 w-12 rounded-full ring-1 ring-neutral-200' />
...
export function InlineNav({
  ...
  return (
    <nav className='mt-2 text-sm text-neutral-600 flex gap-6'>
      {links.map((l) => {
        const isCurrent = current && l.label.toLowerCase() === current.toLowerCase()
        if (l.to) {
          return (
            <Link
              key={l.label}
              to={l.to}
              className={
                isCurrent ? 'underline underline-offset-4' : 'hover:underline underline-offset-4'
              }
            >
              {l.label}
            </Link>
...
export function Callout({
  ...
  return (
    <div className={`rounded-lg border p-4 text-[15px] leading-7 ${toneClasses[tone]}`}>
      {children}
    </div>
  )
}
...
export function Section({ title, children }: { title?: string; children: React.ReactNode }) {
  return (
    <section className='space-y-3'>
      {title ? <h2 className='text-[15px] font-medium text-neutral-900'>{title}</h2> : null}
      {children}
    </section>
  )
}
