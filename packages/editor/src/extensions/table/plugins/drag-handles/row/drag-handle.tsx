    ...
    <>
      <div className='table-row-handle-container absolute z-20 top-0 left-0 flex justify-center items-center h-full -translate-x-1/2'>
        <button
          ...
          className={cn(
            'py-1 bg-muted border border-border/80 rounded-sm outline-none transition-all duration-200',
            {
              '!opacity-100 bg-primary border-primary': isDropdownOpen,
              'hover:bg-muted/80': !isDropdownOpen,
            }
          )}
        >
          <Ellipsis className='size-4 text-foreground rotate-90' />
        </button>
      </div>
      {isDropdownOpen && (
        <FloatingPortal>
          ...
          />
          <div
            className='max-h-[90vh] w-[12rem] overflow-y-auto rounded-md border-[0.5px] border-border bg-card px-2 py-2.5 shadow-md'
