...
const nodes: Record<string, NodeSpec> = {
  ...
  bulletList: {
    ...
    toDOM(node) {
      return [
        'ul',
        { 'data-id': node.attrs.id, class: 'pm-bullet-list ml-6 list-disc space-y-1 my-2' },
        0,
      ]
    },
  },
    ...
    toDOM(node) {
      return [
        'blockquote',
        {
          'data-id': node.attrs.id,
          class:
            'pm-blockquote border-l-4 border-gray-300 dark:border-gray-600 pl-4 my-3 italic text-gray-700 dark:text-gray-300',
        },
        0,
      ]
    },
    ...
    toDOM(node) {
      return [
        'pre',
        {
          ...
          class:
            'pm-code-block bg-gray-100 dark:bg-gray-800 rounded-md p-4 my-3 overflow-x-auto font-mono text-sm',
        },
        ['code', 0],
      ]
    },
    ...
    toDOM(node) {
      return [
        'a',
        {
          ...
          class:
            'pm-note-link text-purple-600 dark:text-purple-400 underline cursor-pointer hover:text-purple-800 dark:hover:text-purple-300',
        },
        0,
      ]
    },
...
export const getDefaultDoc = () => {
  return schema.node('doc', null, [schema.node('paragraph', { id: createNodeId() })])
}
