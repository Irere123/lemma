...
export type DocumentStoreApi = {
  ...
  updateDocument: (update: { id: string; content?: Descendant[]; title?: string; subtitle?: string }) => void;
  subscribe: (listener: () => void) => () => void;
};
...
const DefaultTooltip: ComponentType<TooltipProps> = ({ children }) => <div className="relative inline-block">{children}</div>;
const DefaultTooltipTrigger: ComponentType<{ children: ReactNode; asChild?: boolean }> = ({ children }) => <>{children}</>;
const DefaultTooltipContent: ComponentType<{ children: ReactNode }> = ({ children }) => (
  <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap">{children}</span>
);
const DefaultTooltipProvider: ComponentType<{ children: ReactNode; delayDuration?: number }> = ({ children }) => <>{children}</>;

const DefaultDropdownMenu: ComponentType<DropdownMenuProps> = ({ children }) => <div className="relative">{children}</div>;
const DefaultDropdownMenuTrigger: ComponentType<DropdownMenuTriggerProps> = ({ children }) => <>{children}</>;
const DefaultDropdownMenuContent: ComponentType<DropdownMenuContentProps> = ({ children, className }) => (
  <div className={`absolute z-50 bg-white dark:bg-gray-800 border rounded shadow-lg ${className || ""}`}>{children}</div>
);
const DefaultDropdownMenuItem: ComponentType<DropdownMenuItemProps> = ({ children, onClick, className }) => (
  <button onClick={onClick} className={`w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700 ${className || ""}`}>
    {children}
  </button>
);
...
export function EditorProvider(props: EditorProviderProps) {
  ...
  const value = useMemo<EditorContextValue>(
    ...
    [ui, uploadImage, imageExtensions, editorStore, documentStore, showToast, createEditor, callbacks]
  );
