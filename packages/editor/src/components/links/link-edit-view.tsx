...
function InputView({ label, value, placeholder, onChange, autoFocus }: InputViewProps) {
  return (
    <div className='flex flex-col gap-1'>
      <label htmlFor='input' className='inline-block font-semibold text-xs text-muted-foreground'>
        {label}
      </label>
      <input
        id='input'
        ...
        className='w-[280px] outline-none bg-muted text-foreground text-sm border border-border rounded-md p-2'
        value={value}
...
export function LinkEditView({ viewProps }: LinkEditViewProps) {
  ...
  return (
    <div
      ...
      className='shadow-md rounded-sm p-2 flex flex-col gap-3 bg-muted border-border/50 border-2 animate-in fade-in translate-y-1'
      style={{
    ...
    >
      ...
      <div className='flex text-sm text-muted-foreground gap-2 items-center'>
        ...
        <button
          onClick={removeLink}
          className='cursor-pointer hover:text-placeholder transition-colors'
        >
          Remove Link
        </button>
      </div>
