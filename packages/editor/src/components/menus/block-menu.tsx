...
import { CopyIcon, type LucideIcon, TrashIcon } from 'lucide-react'
  ...
  return (
    <FloatingPortal>
      <div
        ...
        className={cn(
          'max-h-60 min-w-[7rem] overflow-y-scroll rounded-lg border border-border/50 bg-card p-1.5 shadow-md',
        ...
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.key}
              type='button'
              className='flex w-full items-center gap-1.5 truncate rounded-sm px-1 py-1.5 text-xs text-muted-foreground hover:bg-muted'
              onClick={(e) => {
                item.onClick(e)
                e.preventDefault()
                e.stopPropagation()
