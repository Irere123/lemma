...
export function BubbleMenuNodeSelector(props: Props) {
  ...
  return (
    ...
    >
      <section className='w-48 max-h-[90vh] mt-1 flex flex-col overflow-y-scroll rounded-md border-[0.5px] border-border bg-card px-2 py-2.5 shadow-md'>
        {items.map((item) => (
          <button
            key={item.name}
            type='button'
            ...
            className={cn(
              'flex items-center justify-between rounded-sm px-1 py-1.5 text-sm text-muted-foreground hover:bg-muted',
              {
                'bg-muted': activeItem.name === item.name,
              }
            )}
          >
            ...
            {activeItem.name === item.name && (
              <CheckIcon className='size-3 text-muted-foreground flex-shrink-0' />
            )}
          </button>
        ))}
      </section>
