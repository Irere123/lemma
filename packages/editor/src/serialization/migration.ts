...
function migrateNode(node: SlateElement | SlateText, s: Schema): ProseMirrorNode | null {
  // Handle text nodes
  if ('text' in node) {
  ...
  switch (element.type) {
    ...
    case SlateElementType.ListItem:
      ...
      const listItemParagraph = s.nodes.paragraph.create({ id: createNodeId() }, listItemContent)
      return s.nodes.listItem.create({ id: element.id || createNodeId() }, listItemParagraph)

    case SlateElementType.CheckListItem:
      const taskItemContent = migrateInlineContent(children, s)
      const taskItemParagraph = s.nodes.paragraph.create({ id: createNodeId() }, taskItemContent)
    ...
    case SlateElementType.Blockquote:
      ...
      if (quoteBlocks.length === 0 || quoteBlocks.every((n) => n.isText)) {
        ...
        return s.nodes.blockquote.create({ id: element.id || createNodeId() }, quoteParagraph)
      }
      return s.nodes.blockquote.create({ id: element.id || createNodeId() }, quoteBlocks)
...
function migrateListItems(children: (SlateElement | SlateText)[], s: Schema): ProseMirrorNode[] {
  return children
    .map((child) => {
      if ('text' in child) {
        ...
        const paragraph = s.nodes.paragraph.create(
          { id: createNodeId() },
          [(child as SlateText).text ? s.text((child as SlateText).text) : null].filter(
            Boolean
          ) as ProseMirrorNode[]
        )
