...
export function fromMarkdown(markdown: string, s: Schema = defaultSchema): ProseMirrorNode {
  ...
  while (i < lines.length) {
    ...
    if (headingMatch) {
      ...
      blocks.push(s.nodes.heading.create({ level, id: createNodeId() }, content))
      i++
      continue
    }
    ...
    if (trimmedLine.startsWith('> ')) {
      ...
      const quoteParagraphs = quoteContent
        .split('\n\n')
        .filter(Boolean)
        .map((text) => s.nodes.paragraph.create({ id: createNodeId() }, parseInline(text, s)))
    ...
    if (/^- \[[ x]\]\s/.test(trimmedLine)) {
      ...
      while (i < lines.length && /^- \[[ x]\]\s/.test(lines[i].trim())) {
        ...
        if (match) {
          ...
          taskItems.push(s.nodes.taskItem.create({ checked, id: createNodeId() }, itemParagraph))
        }
        i++
      }
    ...
    if (paragraphLines.length > 0) {
      ...
      blocks.push(s.nodes.paragraph.create({ id: createNodeId() }, parseInline(text, s)))
    }
  }
