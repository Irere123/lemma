...
export function setCodeBlock(language?: string): Command {
  return (state, dispatch, view) => {
    return setBlockType(schema.nodes.codeBlock, { language, id: createNodeId() })(state, dispatch, view);
  };
}
...
export const insertDivider: Command = (state, dispatch) => {
  ...
  if (dispatch) {
    const { $from } = state.selection;
    const tr = state.tr
      .replaceWith($from.pos, $from.pos, divider)
      .insert($from.pos + 1, paragraph);
 ...
 */
export function isNodeActive(state: EditorState, nodeType: NodeType, attrs?: Record<string, any>): boolean {
  const { $from, to } = state.selection;
