...
export function DocumentMoreDropdown({ documentId }: Props) {
  ...
  const deleteDocumentMutation = useMutation(
    trpc.documents.deleteDocument.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries(trpc.documents.getUserDocuments.queryOptions({}))
      },
    })
  )
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent sideOffset={10} align='end'>
        ...
        <DropdownMenuItem onSelect={handleOpenConfirm} disabled={deleteDocumentMutation.isPending}>
          <IconTrash />
          Delete
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem className='flex items-center text-xs'>
          ...
          <p className='text-gray-700'>{format(new Date(document.updatedAt!), 'PPp')}</p>
        </DropdownMenuItem>
        <DropdownMenuItem className='flex items-center text-xs'>
          <p className='text-gray-500'>Created</p>
          <p className=' text-gray-700'>{format(new Date(document.createdAt!), 'PPp')}</p>
        </DropdownMenuItem>
      </DropdownMenuContent>
      <ConfirmDialog
        open={isConfirmOpen}
