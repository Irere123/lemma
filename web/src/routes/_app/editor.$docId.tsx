...
import { documentStore, useDocumentStore, type DocumentUpdate } from '@/stores/document-store'
...
function RouteComponent() {
  ...
  const { mutateAsync: updateDbDocument, isPending: isUpsertLoading } = useMutation(
  ...
  const isSynced = useMemo(
    () => syncState.isTitleSynced && syncState.isContentSynced && syncState.isSubtitleSynced,
    [syncState]
  )
  ...
  const handleDocumentUpdate = useCallback(
    async (documentUpdate: DocumentUpdate) => {
      ...
      await queryClient.invalidateQueries({
        queryKey: [['documents', 'getDocumentById'], { input: { id: documentId } }],
      })
    ...
    [updateDbDocument, document?.status, documentId, updateDocument, queryClient]
  )
  ...
  useEffect(() => {
    ...
    if (documentUpdate.title || documentUpdate.content || documentUpdate.subtitle) {
      const handler = setTimeout(() => handleDocumentUpdate(documentUpdate), SYNC_DEBOUNCE_MS)
        ...
        <div className='mx-auto flex w-full flex-1 flex-col md:w-lg lg:w-160 xl:w-3xl'>
          <div className='px-8 pt-8 pb-1 md:px-12 md:pt-12'>
            ...
            <div className='mt-4 text-gray-600'>
              {documentError instanceof Error ? documentError.message : 'An unknown error occurred'}
            </div>
          </div>
        </div>
      </div>
